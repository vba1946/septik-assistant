...
TIER_CONFIG = {
    'simple': {
        ...
        'exhaust_msg': 'Доступ к консультанту уже предоставлялся, могу предложить возможность передать ваши контакты менеджеру через форму обратной связи.',
    },
    'demo': {
        'label': 'Demo',
        'maxq': 18,
        ...
        'exhaust_msg': 'Демо-доступ исчерпан (18 вопросов). Если хотите продолжить консультацию с менеджером, оставьте свои контакты (имя, телефон).',
    },
    'pro': {
        'label': 'PRO',
        'maxq': 10,
...
Если вопрос не относится к данной тематике — ты обязан сообщить об ограничении компетенции и предложить оставить контакты:
«Извините, я — консультант по автономной канализации и отвечаю только на вопросы, связанные с этой темой. Пожалуйста, задайте вопрос по выбору типа автономной канализации, ее монтажу и установке. Если эти вопросы важны для вас, оставьте свои контакты (имя, телефон) — менеджер свяжется с вами и ответит на все вопросы.»
...
«Я не называю конкретные цены, бренды и модели. Моя задача — помочь подобрать тип автономной канализации под ваши условия. Если эти вопросы важны для вас, оставьте свои контакты (имя, телефон) — менеджер свяжется с вами и ответит на все вопросы.»
...
def contact():
    ...
    conn.commit()
    conn.close()
    return jsonify({'ok': True, 'message': 'Ваши данные переданы менеджеру. Вы можете также лично связаться с менеджером, позвонив ему по указанному телефону на сайте компании.'})
